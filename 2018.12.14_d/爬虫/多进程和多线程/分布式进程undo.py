...
分布式进程在 Python 中依然要用到 multiprocessing 模块。
multiprocessing 模块不但支持多进程，
其中 managers 子模块还支持把多进程分布到多台机器上。
可以写一个服务进程作为调度者，将任务分布到其他多个进程中，依靠网络通信进行管理。
举个例子：在做爬虫程序时，常常会遇到这样的场景，
我们想抓取某个网站的所有图片，如果使用多进程的话，
一般是一个进程负责抓取图片的链接地址，将链接地址存放到 Queue 中，
另外的进程负责从 Queue 中读取链接地址进行下载和存储到本地。
现在把这个过程做成分布式，一台机器上的进程负责抓取链接，其他机器上的进程负责下载存储。
那么遇到的主要问题是将 Queue 暴露到网络中，让其他机器进程都可以访问，
分布式进程就是将这一个过程进行了封装，我们可以将这个过程称为本地队列的网络化。整体过程如图1-24所示。
'''
'''图1-24　分布式进程

要实现上面例子的功能，创建分布式进程需要分为六个步骤：

1）建立队列 Queue，用来进行进程间的通信。
服务进程创建任务队列 task_queue，用来作为传递任务给任务进程的通道；
服务进程创建结果队列 result_queue，作为任务进程完成任务后回复服务进程的通道。
在分布式多进程环境下，必须通过由 Queuemanager 获得的 Queue 接口来添加任务。
...
# 第一步：建立 task_queue 和 result_queue，用来存放任务和结果(linux)
...
'''
manager.shutdown()'''
# 服务进程已经编写完成，接下来编写任务进程（taskWorker.py），创建任务进程的步骤相对较少，需要四个步骤：
...
'''程序 taskWorker.py 代码（win/linux 版）如下：
'''
     # coding:utf-8
     import time
     from multiprocessing.managers import BaseManager
     ...
     # 第四步：从 task 队列获取任务,并把结果写入 result 队列:
...
最后开始运行程序，先启动服务进程 taskManager.py，运行结果如下：
     ...
     put task ImageUrl_9 ...
     try get result...
接着再启动任务进程 taskWorker.py，运行结果如下：
...
其实这就是一个简单但真正的分布式计算，把代码稍加改造，启动多个 worker，就可以把任务分布到几台甚至几十台机器上，实现大规模的分布式爬虫。

注意　

由于平台的特性，创建服务进程的代码在 Linux 和 Windows 上有一些不同，创建工作进程的代码是一致的。'''
taskManager.py 程序在 Windows 版下的代码如下：
     ...
     class QueueManager(BaseManager):
        pass
     def win_run():
        # Windows 下绑定调用接口不能使用 lambda，所以只能先定义函数再绑定
        QueueManager.register('get_task_queue',callable = get_task)
        QueueManager.register('get_result_queue',callable = get_result)
        # 绑定端口并设置验证口令，Windows 下需要填写IP地址，Linux 下不填默认为本地

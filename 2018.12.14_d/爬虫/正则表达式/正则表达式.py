...
上一节讲解了正则表达式的语法和应用，对于不同的编程语言来说，对正则表达式的语法绝大部分语言都是支持的，但是还是略有不同，
每种编程语言都有一些独特的匹配规则，Python 也不例外。下面通过表4-7列出一些 Python 的匹配规则。

表4-7　Python 的匹配规则



在讲 Python 对正则表达式的实现之前，首先让说一下反斜杠问题。正则表达式里使用“\”作为转义字符，这就可能造成反斜杠困扰。假如你需要匹配文本中的字符“\”，那么使用编程语言表示的正则表达式里将需要4个反斜杠“\\”：前两个和后两个分别用于在编程语言里转义成反斜杠，转换成两个反斜杠后再在正则表达式里转义成一个反斜杠。但是 Python 提供了对原生字符串的支持，从而解决了这个问题。匹配一个‘\’的正则表达式可以写为r‘\’，同样，匹配一个数字的‘\d’可以写成 r‘\d’，

Python 通过 re 模块提供对正则表达式的支持。使用 re 的一般步骤是先将正则表达式的字符串形式编译为 Pattern 实例，然后使用 Pattern 实例处理文本并获得匹配结果，最后使用 Match 实例获得信息，进行其他操作。主要用到的方法列举如下：
...
首先说一下 re 中 compile 函数，它将一个正则表达式的字符串转化为 Pattern 匹配对象。示例如下：

     pattern = re.compile(r'\d+')
这会生成一个匹配数字的 pattern 对象，用来给接下来的函数作为参数，进行进一步的搜索操作。

大家发现其他几个函数中，还有一个 flag 参数。参数 flag 是匹配模式，取值可以使用按位或运算符“|”表示同时生效，比如 re.I|re.M。flag 的可选值如下：
...
运行结果如下：
     ...
     192
     匹配失败2
匹配 192abc 字符串时，match 函数是从字符串开头进行匹配，匹配到192立即返回值，通过 group（） 可以获取捕获的值。同样，匹配 abc192 字符串时，字符串开头不符合正则表达式，立即返回 None。

2.re.search（pattern，string[，flags]）

search 方法与 match 方法极其类似，区别在于 match（） 函数只从 string 的开始位置匹配，search（） 会扫描整个 string 查找匹配，match（） 只有在 string 起始位置匹配成功的时候才有返回，如果不是开始位置匹配成功的话，match（） 就返回 None。search 方法的返回对象和 match（） 返回对象在方法和属性上是一致的。示例如下：
...
按照能够匹配的子串将 string 分割后返回列表。maxsplit 用于指定最大分割次数，不指定，则将全部分割。示例如下：
...
搜索整个 string，以列表形式返回能匹配的全部子串。示例如下：
...
使用 repl 替换 string 中每一个匹配的子串后返回替换后的字符串。当 repl 是一个字符串时，可以使用 \id 或\g<id>、\g<name>引用分组，但不能使用编号0。当 repl 是一个方法时，这个方法应当只接受一个参数（Match 对象），并返回一个字符串用于替换（返回的字符串中不能再引用分组）。count 用于指定最多替换次数，不指定时全部替换。示例如下：
...
输出结果为：
     ...
     ('say i, world hello!', 2)
     ('I Say, Hello World!', 2)
以上7个函数在 re 模块中进行搜索匹配，如何将捕获到的值提取出来呢？这就需要用到 Match 对象，之前已经使用了 Match 中的 groups 方法，现在介绍一下 Match 对象的属性和方法。
...
end（[group]）：返回指定的组截获的子串在 string 中的结束索引（子串最后一个字符的索引+1）。group 默认值为0。

span（[group]）：返回（start（group），end（group））。

expand（template）：将匹配到的分组代入 template 中然后返回。template 中可以使用 \id 或 \g<id>、\g<name>引用分组，但不能使用编号0。\id 与\g<id>是等价的，但\10将被认为是第10个分组，如果你想表达\1之后是字符‘0’，只能使用\g<1>0。
...
前文介绍的7种方法的调用方式大都是 re.match、re.search 之类，其实还可以使用由 re.compile 方法产生的 Pattern 对象直接调用这些函数，类似 pattern.match，pattern.search，只不过不用将 Pattern 作为第一个参数传入。函数对比如表4-8所示。
